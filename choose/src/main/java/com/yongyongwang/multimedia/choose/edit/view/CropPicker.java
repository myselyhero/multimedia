...
public class CropPicker extends ImagePicker {
    ...
    private void initConstants() {
        touchTolerance = dip2px(TOUCH_TOLERANCE);
        minSelectionLength = dip2px(MIN_SELECTION_LENGTH);
        borderThickness = dip2px(BOARDER_THICKNESS);
        cornerThickness = dip2px(CORNER_THICKNESS);
        cornerLength = dip2px(CORNER_LENGTH);
    }
     ...
     */
    public void setAspectRatio(float ratio, RectF boundRatio) {
    ...
    private void moveEdges(MotionEvent event, RectF highlightRect) {
        ...
        if ((movingEdges & MOVE_BLOCK) != 0) {
            dx = clamp(dx, -highlightRect.left, 1 - highlightRect.right);
            dy = clamp(dy, -highlightRect.top, 1 - highlightRect.bottom);
            ...
            float left = clamp(highlightRect.left + minSelectionLength /
                    baseLineRect.width(), 0.0f, 1.0f);
            float right = clamp(highlightRect.right - minSelectionLength /
                    baseLineRect.width(), 0.0f, 1.0f);
            float top = clamp(highlightRect.top + minSelectionLength /
                    baseLineRect.height(), 0.0f, 1.0f);
            float bottom = clamp(highlightRect.bottom - minSelectionLength /
                    baseLineRect.height(), 0.0f, 1.0f);
            if ((movingEdges & MOVE_RIGHT) != 0) {
                highlightRect.right = clamp(ratioX, left, 1f);
            }
            if ((movingEdges & MOVE_LEFT) != 0) {
                highlightRect.left = clamp(ratioX, 0, right);
            }
            if ((movingEdges & MOVE_TOP) != 0) {
                highlightRect.top = clamp(ratioY, 0, bottom);
            }
            if ((movingEdges & MOVE_BOTTOM) != 0) {
                highlightRect.bottom = clamp(ratioY, top, 1f);
            }
            if (aspectRatio != UNSPECIFIED) {
    ...
    private void moveEdgesWithSpecifiedRatio(
        ...
        if (isGreaterThanTargetRatio) {
            ...
            if ((movingEdges & MOVE_BOTTOM) != 0) {
                highlightRect.bottom = clamp(highlightRect.top + height, top, maxThreshHold);
            } else {
                highlightRect.top = clamp(highlightRect.bottom - height, minThreshHold, bottom);
            }
        } else {
            float width = highlightRect.height() * targetRatio;
            if ((movingEdges & MOVE_LEFT) != 0) {
                highlightRect.left = clamp(highlightRect.right - width, minThreshHold, right);
            } else {
                highlightRect.right = clamp(highlightRect.left + width, left, maxThreshHold);
            }
        }
        if (isGreaterThanTargetRatio) {
            float width = highlightRect.height() * targetRatio;
    ...
    private void drawHighlightRectangle(Canvas canvas, RectF hlightRectF) {
        ...
        tmpPaint.setColor(Color.blue(COLOR_SHADOW));
